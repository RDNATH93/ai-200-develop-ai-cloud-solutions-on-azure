...
create_cosmosdb_account() {
    ...
    local account_exists=$(az cosmosdb show --resource-group $rg --name $account_name)
    ...
    local db_exists=$(az cosmosdb sql database show --resource-group $rg --account-name $account_name --name $database_name)
    if [ -z "$db_exists" ]; then
    ...
    local container_exists=$(az cosmosdb sql container show --resource-group $rg --account-name $account_name --database-name $database_name --name $container_name)
