...
rg="esource-group"  # Resource Group name
...
create_acr_and_build_image() {
    ...
    docker build -t $acr_name.azurecr.io/$container_image --file api/Dockerfile 
...
check_deployment_status() {
    ...
    echo ""
    echo "App Service Plan ($app_plan):"
    local plan_status=$(az appservice plan show --resource-group $rg --name $app_plan --query "properties.provisioningState" -o tsv 2>/dev/null)
